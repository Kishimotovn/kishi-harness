...
One in-flight operation owns cancellation and draining. The command reserves its pending promise before entering tool middleware, so synchronous reentry cannot start a second tool. Session disposal, MCP closure, client cancellation, and the configured deadline cancel the call. A missing owner, unrelated tool, or overlapping call is rejected. The Web server must bind to `127.0.0.1`; no additional HTTP listener or MCP authentication scheme is introduced.
...
Source-only tests cover schema discovery, logged execution, tool denial, overlapping calls, cancellation, timeout, missing Session, configuration admission, and rollback of acquired resources. The real Web-profile smoke belongs to the built-bin gate because Client modules require emitted browser assets; it does not run in source-only coverage. The [tooling task](https://github.com/Kishimotovn/kishi-harness/issues/17) records live editor discovery, human-approved Client activation, and removal verified through the Client slot registry. The native browser test separately verifies a rendered Client-slot change that remains absent before approval, disappears on stop, and preserves the composer.
