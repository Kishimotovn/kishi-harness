...
Run checks before pushes via [dsh-pre-push-checks](.agents/skills/dsh-pre-push-checks/SKILL.md), subject to the [fork's CI-only documentation policy](docs/agents/documentation-policy.md#verification), which overrides upstream local-check requirements. Report only commands run. After `gh stack sync`, validate immediately; do not merge before checks pass.
...
Docs accompany every code change: update affected README and JSDoc contracts together. Follow the [fork documentation policy](docs/agents/documentation-policy.md) for language requirements and [docs/AGENTS.md](docs/AGENTS.md) for structure and checks. `dsh-translate-docs` remains explicitly user-invoked.
