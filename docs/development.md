...
Fork pull requests use standard GitHub-hosted Linux runners with lower parallelism and required Linux/macOS packaged-runtime checks. Windows CI and upstream runner-fleet drills stay defined but run only upstream; the [fork platform policy](../.agents/notes/implemented/process/2026-09-16-fork-linux-macos-ci.md) defines the exact scope. Upstream Project automation is skipped; the preview still builds but does not deploy to Cloudflare or post a deployment URL. Existing failover switches remain explicit operator selections. See the [fork PR CI decision](../.agents/notes/implemented/process/2026-09-13-fork-pull-request-ci.md) for infrastructure ownership and verification limits.
