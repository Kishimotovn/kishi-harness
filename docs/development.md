...
Fork pull requests default to standard GitHub-hosted Linux and Windows runners with lower parallelism and unchanged code checks. Upstream Project automation is skipped; the preview still builds but does not deploy to Cloudflare or post a deployment URL. Existing failover switches remain explicit operator selections. See the [fork PR CI decision](../.agents/notes/implemented/process/2026-09-13-fork-pull-request-ci.md) for scope and verification limits.
