...
describe('Session format release authority', () => {
  it('keeps maintained release records consistent with the writer and evidence links', () => {
    const files = ['docs/session-format-status.md', 'docs/session-format-status.zh.md']
      .filter(file => isMaintainedDoc(file, englishOnly))
    const records = files.map(file =>
      validateSessionFormatRelease(readFileSync(resolve(root, file), 'utf8'), readCurrentSessionFormatVersion(root)),
    )
    expect(records).toHaveLength(englishOnly ? 1 : 2)
